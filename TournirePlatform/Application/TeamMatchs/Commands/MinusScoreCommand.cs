...
public class MinusScoreCommand : IRequest<Result<Domain.TeamsMatchs.TeamMatch, TeamMatchException>>
{
    public Guid TeamId { get; init; }
    public Guid MatchId { get; init; }
}

public class MinusScoreCommandHandler : IRequestHandler<MinusScoreCommand, Result<Domain.TeamsMatchs.TeamMatch, TeamMatchException>>
{
    private readonly IMatchQueries _matchQueries;
    private readonly ITeamMatchRepository _teamMatchRepository;

    public MinusScoreCommandHandler(IMatchQueries matchQueries, ITeamMatchRepository teamMatchRepository)
    {
        _matchQueries = matchQueries;
        _teamMatchRepository = teamMatchRepository;
    }

    public async Task<Result<Domain.TeamsMatchs.TeamMatch, TeamMatchException>> Handle(MinusScoreCommand request, CancellationToken cancellationToken)
    {
        ...
        return await matchOption.Match(
            async match =>
            {
                ...
                if (targetTeamMatch.Score == -1)
                {
                   throw new DivideByZeroException("Score is 0");
                }
